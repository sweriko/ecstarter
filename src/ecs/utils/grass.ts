...
/**
 * Shader Manager for managing GLSL shader code
 */
class ShaderManager {
  ...
  private initShaders(): void {
    ...
    const grassVertexShader = `
      ...
      void main() {
        ...
        // We no longer need to calculate procedural colors as we're using texture directly
        // Just keep the remaining parameters
...
/**
 * Simple math utility functions for grass system
 */
export class MathUtils {
  ...
  static sat(x: number): number {
    return Math.min(Math.max(x, 0.0), 1.0);
  }

  static inRange(x: number, a: number, b: number): boolean {
    return x >= a && x <= b;
  }

  static easeOut(x: number, t: number): number {
    return 1.0 - Math.pow(1.0 - x, t);
  }

  static easeIn(x: number, t: number): number {
    return Math.pow(x, t);
  }

  static randRange(a: number, b: number): number {
    return this.rng() * (b - a) + a;
  }

  static randNormalish(): number {
    const r = this.rng() + this.rng() + this.rng() + this.rng();
    return (r / 4.0) * 2.0 - 1;
  }

  static randInt(a: number, b: number): number {
    return Math.round(this.rng() * (b - a) + a);
  }

  static lerp(x: number, a: number, b: number): number {
    return x * (b - a) + a;
  }

  static smoothstep(edge0: number, edge1: number, x: number): number {
    const t = this.sat((x - edge0) / (edge1 - edge0));
...
/**
 * GrassComponent class that handles grass rendering
 */
export class GrassComponent {
  ...
  private createGrassGeometry(segments: number): THREE.InstancedBufferGeometry {
    ...
    for (let i = 0; i < NUM_GRASS; ++i) {
      offsets.push(MathUtils.randRange(-GRASS_PATCH_SIZE * 0.5, GRASS_PATCH_SIZE * 0.5));
      offsets.push(MathUtils.randRange(-GRASS_PATCH_SIZE * 0.5, GRASS_PATCH_SIZE * 0.5));
      offsets.push(0);
    }
